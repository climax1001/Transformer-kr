...
class TrainManager:
    def __init__(self, model:Model, config:dict, test=False) -> None:
        ...
        self.batch_multiplier = train_config.get("batch_multoplier", 1)
        ...
        if model_continue:
            ...
            if ckpt is None:
                self.logger.info("cant find checkpoint")
            else:
        ...
        for epoch_no in range(self.epochs):
            ...
            if self.gaussian_noise:
                if len(all_epoch_noise) != 0:
                    self.model.out_stds = torch.mean(torch.stack(([noise.std(dim=[0]) for noise in all_epoch_noise])),
                                                     dim=-2)
                else:
